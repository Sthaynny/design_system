...
/// Parent data structure used by [DSRenderStaggeredGrid].
class DSStaggeredGridParentData extends ContainerBoxParentData<RenderBox> {
...
abstract class DSStaggeredGridDelegate {
  /// Abstract const constructor. This constructor enables subclasses to provide
  /// const constructors so that they can be used in const expressions.
  const DSStaggeredGridDelegate();
  ...
  /// be different.
  bool shouldRelayout(covariant DSStaggeredGridDelegate oldDelegate);
}
...
class DSStaggeredGridDelegateWithFixedCrossAxisCount
    extends DSStaggeredGridDelegate {
  /// Creates a delegate that makes grid layouts with a fixed number of tiles in
  /// the cross axis.
  ///
  /// The [crossAxisCount] argument must be greater than zero.
  const DSStaggeredGridDelegateWithFixedCrossAxisCount({
  ...
  @override
  bool shouldRelayout(
    DSStaggeredGridDelegateWithFixedCrossAxisCount oldDelegate,
  ) {
...
class DSStaggeredGridDelegateWithMaxCrossAxisExtent
    extends DSStaggeredGridDelegate {
  /// Creates a delegate that makes grid layouts with tiles that have a maximum
  /// cross-axis extent.
  ///
  /// The [maxCrossAxisExtent] argument must be greater than zero.
  const DSStaggeredGridDelegateWithMaxCrossAxisExtent({
  ...
  @override
  bool shouldRelayout(
    DSStaggeredGridDelegateWithMaxCrossAxisExtent oldDelegate,
  ) {
...
class DSRenderStaggeredGrid extends RenderBox
    with
        ContainerRenderObjectMixin<RenderBox, DSStaggeredGridParentData>,
        RenderBoxContainerDefaultsMixin<RenderBox, DSStaggeredGridParentData> {
  /// Creates a [DSRenderStaggeredGrid].
  DSRenderStaggeredGrid({
    List<RenderBox>? children,
    required DSStaggeredGridDelegate delegate,
  ...
  DSStaggeredGridDelegate get delegate => _delegate;
  DSStaggeredGridDelegate _delegate;
  set delegate(DSStaggeredGridDelegate newDelegate) {
  ...
  void setupParentData(RenderObject child) {
    if (child.parentData is! DSStaggeredGridParentData) {
      child.parentData = DSStaggeredGridParentData();
    }
  }

  DSStaggeredGridParentData _getParentData(RenderBox child) {
    return child.parentData as DSStaggeredGridParentData;
  }
  ...
  Size _computeSize({
    ...
    int computeCrossAxisCellCount(
      DSStaggeredGridParentData childParentData,
      int crossAxisCount,
    ) {
      return math.min(
